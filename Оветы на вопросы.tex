...
\subsection*{13. Энергия электрического поля. Плотность энергии электрического поля.}

\imc[0.4\textwidth]{23.png}

\fc{A_{12}=W_{12}=q_2\varphi_{1B}=q_1\varphi_{2A}}
\fc{W_{12}=\frac{1}{2}(q\varphi_A+q_2\varphi_B)}
\fc{W=W_1+W_2+W_{12}}

\newpage

\imc[0.4\textwidth]{24.png}

\fc{W_{12}=q_1(\varphi_{2A}+\varphi_{3A}+q_2(\varphi_{1B}+\varphi_{2B})+q_3(\varphi_{1C}+\varphi_{2C})}
\fc{W_{12}=\frac{1}{2}(q_1\varphi_1+q_2\varphi_2+q_3\varphi_3)}

Итог:

\fc{W_{\text{вз}}=\frac{1}{2}\Sigma q_i\varphi_i=\frac{1}{2}\int\varphi dq}

\imc[0.4\textwidth]{25.png}

\fc{W_{\text{эл}}=\underset{V}{\int}W_{\text{эл}}dV=\int \frac{E^2}{4\pi}dV}

Если $q=\rho dV$ :

\fc{W=\frac{1}{2}\int\varphi\rho dV=[\Delta\varphi=-4\pi\rho]=-\frac{1}{8\pi}\int (\Delta\varphi\cdot\varphi)dV=}
\fc{=[\mm{div}(\varphi\grad\varphi=\varphi\Delta\varphi+(\grad\varphi)^2]=}
\fc{=-\frac{1}{8\pi}\int\underset{\rightarrow0}{\mm{div}(\varphi\Delta\varphi )dV}+\frac{1}{8\pi}\int (\grad\varphi)^2dV=\int \frac{E^2}{8\pi}dV=\int wdV}
где $w=\frac{E^2}{8\pi}-$\kr{плотность энергии}
