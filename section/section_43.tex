...
Ищем решение в виде : \( \vec{E}=E_0\vec{e}_x e^{-i\omega t} e^{ikz}   \)
...
\[
\vec{j}_n\overset{df}{=}-D\grad n \qquad 3)\frac{\partial n}{\partial t} +\mathrm{div}\vec{j}_n=0 
\]
...
\begin{minipage}[c]{0.6\textwidth} % Правая часть: текст
   \[
   1)\frac{dN}{dt}=-\oiint \vec{j}_nd\vec{S}
   \]
\end{minipage}

\begin{gather*}
    3)\underset{\mathbb{V}}{\iiint} \frac{\partial n}{\partial t} dV=-\underset{\mathbb{V}}{\iiint}\mathrm{div}\vec{j}_n \, dV \Rightarrow \frac{\partial n}{\partial t}+\mathrm{div}\vec{j}_n=0, \\
